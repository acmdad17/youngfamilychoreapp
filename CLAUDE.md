# Young Family Chore App — Claude Code Context

## How Handoffs Work
- **Cowork → Claude Code**: Check `TASK.md` for the current task. Do it, write results/status to `RESULT.md`, delete `TASK.md` when done.
- **Claude Code → Cowork**: Write output to `RESULT.md`. Cowork (Bradford's AI assistant) will read it on the next turn.
...
**Deploy rule**: Bradford pushes manually — never `git push` via Claude Code without being asked. Run `git add -A && git commit -m "message" && git push` from this folder.
